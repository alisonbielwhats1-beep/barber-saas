...
import { PWA_BACKGROUND } from "../src/lib/pwa-icons";
...
async function main() {
  ...
  const mark = await sharp({ create: { width: 288, height: 288, channels: 3, background: "#c7b4e5" } })
    .joinChannel(alpha).png().toBuffer();
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512"><rect width="512" height="512" fill="${PWA_BACKGROUND}"/><image x="112" y="112" width="288" height="288" href="data:image/png;base64,${mark.toString("base64")}"/></svg>`;
  ...
  await Promise.all(
    variants.map(({ filename, size }) =>
      sharp(Buffer.from(svg))
        .resize(size, size)
        .flatten({ background: PWA_BACKGROUND })
