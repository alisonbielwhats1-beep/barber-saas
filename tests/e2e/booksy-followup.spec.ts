...
test("@database busca completa, encaixe ao editar e bloqueio editável até meia-noite", async ({ page }) => {
  ...
  try {
    ...
    await expect(form.getByRole("button").filter({ hasText: clientName })).toHaveCount(0);
    await form.getByLabel("Pesquisar cliente").fill(clientName);
    await expect(form.getByRole("button").filter({ hasText: clientName })).toHaveCount(1);
    await form.getByRole("button").filter({hasText:clientName}).click();
