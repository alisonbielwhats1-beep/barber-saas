...
export function Editor({entity,record,account,options,initial={}}:{entity:string;record?:Row|null;account?:Row|null;options:Options;initial?:Record<string,string>}) {
 ...
 <div className="hq-form-grid">{isAccount&&<><h3 className="hq-form-title">Dados de contato</h3>{definition("accounts").fields.map(f=><FormField key={f.key} field={f} record={account} options={options} prefix="account."/>) }<h3 className="hq-form-title">Informações comerciais</h3></>}{def.fields.filter(f=>!f.readonly&&!(isAccount&&f.key==="accountId")).map(f=><FormField key={f.key} field={f} record={initialRecord} options={options}/>)}</div>
