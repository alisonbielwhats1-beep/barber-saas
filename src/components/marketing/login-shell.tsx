...
export function PremiumLoginShell({ children }: { children: ReactNode }) {
  ...
  return <main id="main-content" tabIndex={-1} className={`mk mk-auth ef-marketing`} data-ready={ready} data-segment={segment.id} data-atmosphere={segment.id === "barbearia" ? "dark" : "light"}>
    ...
    <aside className="mk-auth-visual"><Image key={segment.id} src={segment.image} alt={segment.alt} fill priority quality={85} sizes="(max-width: 900px) 1px, (min-aspect-ratio: 2/1) 55vw, 100vw" style={{ objectPosition: segment.position }} /><div className="mk-auth-visual-copy"><p>O TALENTO É SEU.</p><h2>A tranquilidade de ter<br />tudo em ordem, também.</h2><span>Everflair · {segment.label}</span></div></aside>
  </main>;
}
