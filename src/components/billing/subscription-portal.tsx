...
export function SubscriptionPortal({ salonId, email, timezone, initial, accessBlocked = false }: { salonId: string; email: string; timezone: string; initial?: BillingIntent; accessBlocked?: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    {subscription && <section className="space-y-5 rounded-xl border border-border bg-surface-1 p-5" aria-labelledby="current-subscription">
      ...
      {subscription.state === "UNPAID" && !subscription.cancelledAt && <details className="text-sm"><summary className="cursor-pointer">Não consigo confirmar o pagamento</summary><p className="mt-2">Use uma conta compradora diferente da conta Mercado Pago que recebe os pagamentos do Everflair. A conta recebedora não pode pagar a própria assinatura. Confira também a mensagem exibida pelo Mercado Pago; trocar o plano não resolve uma recusa da conta ou do cartão.</p></details>}
    ...
    {!accessBlocked && (canChoose || canReplacePending) && <section aria-labelledby="choose-subscription"><h2 id="choose-subscription" className="mb-4 text-xl font-semibold">{canReplacePending ? "Escolher outro plano" : "Escolha seu plano"}</h2>{canReplacePending && <p className="mb-4 text-sm text-muted-foreground">Sua contratação ainda não tem pagamento confirmado. Compare os planos abaixo. Antes de contratar outro, vamos encerrar a tentativa anterior no Mercado Pago. Não há diferença proporcional de período pago a calcular.</p>}<PlanPicker initial={initial} disabled={busy} onChoose={setChoice} /></section>}
    ...
    <Dialog open={Boolean(choice)} onOpenChange={open => { if (!open && !busy) setChoice(null); }}><DialogContent><DialogTitle>{canChoose ? "Confirmar contratação" : "Trocar contratação pendente"}</DialogTitle><DialogDescription>{canChoose ? "Revise o valor antes de seguir para o pagamento seguro no Mercado Pago." : "Primeiro, confirme o encerramento da tentativa anterior. O novo pagamento só ficará disponível depois da confirmação do Mercado Pago."}</DialogDescription>
      ...
      {canChoose ? <Button disabled={busy} onClick={() => void subscribe()}>{busy ? "Preparando pagamento…" : "Ir para pagamento"}</Button> : canReplacePending ? <Button disabled={busy} onClick={() => void cancel()}>{busy ? "Enviando pedido…" : "Confirmar cancelamento da tentativa anterior"}</Button> : <div><p role="status">Aguardando confirmação. Atualize a situação antes de continuar; se houve pagamento, a troca deverá usar a cotação do plano ativo.</p><Button variant="outline" disabled={busy} onClick={() => void refresh()}>Atualizar situação</Button></div>}<Button variant="outline" disabled={busy} onClick={() => setChoice(null)}>Voltar aos planos</Button>
