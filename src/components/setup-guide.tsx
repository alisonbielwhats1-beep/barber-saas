...
export function SetupGuide({ steps, resumeHref }: { steps: SetupStep[]; resumeHref?: string }) {
  ...
  return <section className="rounded-2xl border border-primary/25 bg-primary/5 p-5" aria-labelledby="setup-guide-title">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs text-muted-foreground">SEU ESPAÇO, PASSO A PASSO · {complete} DE {steps.length}</p><h2 id="setup-guide-title" className="mt-2 text-lg font-semibold">{resumeHref ? "Continue de onde parou" : `Próximo passo: ${next.label.toLocaleLowerCase("pt-BR")}`}</h2><p className="mt-2 max-w-xl text-sm text-muted-foreground">{resumeHref ? "Retome a etapa salva ou escolha um dos passos abaixo. No final, conheça o aplicativo e seu link para os clientes." : instructions[index]}</p></div><Link href={resumeHref ?? next.href} className="inline-flex min-h-11 items-center gap-3 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">Continuar configuração<ArrowRight size={16} aria-hidden="true" /></Link></div>
