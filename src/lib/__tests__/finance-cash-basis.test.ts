...
describe("finance: separate realization, cash and forecast", () => {
  it("applies selected month boundaries to receipts and expenses without changing tenant scope", async () => {
    ...
    const tx = { appointment: { findMany: vi.fn().mockResolvedValue([]) }, appointmentProduct: { findMany: vi.fn().mockResolvedValue([]) }, payment: { findMany: paymentQuery }, expense: { findMany: expenseQuery } } as unknown as Tx;
    const metrics = await getFinanceMetrics(tx, "salon-a", "30d", "America/Sao_Paulo", {mode:"month",date:"2028-02-15"});
    const bounds = {gte:new Date("2028-02-01T03:00:00Z"),lt:new Date("2028-03-01T03:00:00Z")};
    expect(paymentQuery.mock.calls[0][0].where).toEqual({appointment:{salonId:"salon-a"},paidAt:bounds});
    expect(expenseQuery.mock.calls[0][0].where).toEqual({salonId:"salon-a",dueDate:bounds});
    expect(metrics.cashflow).toHaveLength(29);
  });
