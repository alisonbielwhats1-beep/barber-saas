...
describe("billing contract and access", () => {
  it.each([
    ["INDIVIDUAL",5990,59900,1], ["TEAM",7990,77900,3], ["TEAM_PLUS",9990,95900,5], ["TEAM_MAX",14990,143900,10],
    ...
    expect(quoteContract({plan,cycle:"ANNUAL"})).toMatchObject({amountCents:annual,agendaLimit:agendas,intervalMonths:12,catalogVersion:"2026-09-13"});
  });
  it("prices extras on the server and rejects supplied prices", () => {
    expect(quoteContract({plan:"TEAM_MAX",cycle:"ANNUAL",extraAgendas:2})).toMatchObject({amountCents:172700,agendaLimit:12});
