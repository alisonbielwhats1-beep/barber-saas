...
export const accountProjection = Prisma.sql`
 ...
 (SELECT plan FROM hq_subscriptions WHERE "customerId"=c.id ORDER BY "createdAt" DESC,id DESC LIMIT 1) AS "planLabel",
 (SELECT "amountCents" FROM hq_subscriptions WHERE "customerId"=c.id ORDER BY "createdAt" DESC,id DESC LIMIT 1) AS "planAmount",
 (SELECT interval FROM hq_subscriptions WHERE "customerId"=c.id ORDER BY "createdAt" DESC,id DESC LIMIT 1) AS "planCycle",
 (SELECT to_jsonb(s)->>'billingPaidThrough' FROM hq_subscriptions s WHERE "customerId"=c.id ORDER BY "createdAt" DESC,id DESC LIMIT 1) AS "paidThrough",
...
export async function dashboardMetrics(tx: Tx) {
 ...
 (SELECT COALESCE(sum("amountCents"-COALESCE((to_jsonb(hq_payments)->>'billingRefundedCents')::int,0)),0)::float8 FROM hq_payments WHERE status='Pago' AND date_trunc('month',"paidDate")=date_trunc('month',CURRENT_TIMESTAMP AT TIME ZONE 'America/Sao_Paulo')) AS received,
