...
export default async function SubscriptionPage({ searchParams }: { searchParams: Promise<{ billingPlan?: string; cycle?: string; extraAgendas?: string }> }) {
  ...
  return <div className="space-y-6"><header><Link href="/configuracoes#plano" className="text-sm text-muted-foreground underline">Voltar às configurações</Link><h1 className="mt-3 text-2xl font-semibold">Plano e assinatura</h1><p className="mt-1 text-muted-foreground">{data.salon.name}</p></header>
    ...
    <aside className="rounded-xl border border-border bg-card p-5"><h2 className="font-semibold">Começando agora?</h2><p className="mt-2 text-sm text-muted-foreground">Configure horários, serviços e profissionais. No final do guia, conheça o aplicativo e o link que você compartilha com os clientes.</p><Link href="/onboarding/configuracao" className="mt-3 inline-flex min-h-11 items-center rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">Configurar meu estabelecimento</Link></aside>
  </div>;
}
