...
export function ProfessionalList({ entries }: { entries: Entry[] }) {
  ...
  return <div className="min-w-0 space-y-3">
    ...
    <div aria-label="Lista de profissionais" className="overflow-hidden rounded-2xl border border-border bg-card">{shown.map(entry => {
      ...
      return <button key={entry.id} type="button" onClick={() => setSelectedId(entry.id)} className="flex min-h-20 w-full items-center gap-3 border-b border-border px-4 py-3 text-left last:border-0 hover:bg-card-hover focus-visible:outline focus-visible:outline-2 focus-visible:outline-ring">
        ...
        <span className="min-w-0 flex-1"><span className="block break-words text-sm font-medium">{entry.name}</span><span className="mt-1 block break-words text-xs text-muted-foreground">{entry.subtitle}</span><span className={`mt-1 block text-xs ${entry.active ? "text-success" : "text-muted-foreground"}`}>{entry.active ? "Ativo" : "Inativo"}</span></span>
