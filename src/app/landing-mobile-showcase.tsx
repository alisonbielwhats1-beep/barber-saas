    ...
    <section className="bg-white py-24 sm:py-28">
      <div className="container">
        <div className="mx-auto max-w-3xl text-center">
          <p className="text-xs font-semibold uppercase tracking-[.2em] text-primary">Aplicativo em todos os lados</p>
          <h2 className="mt-4 font-display text-4xl tracking-[-.04em] sm:text-5xl">O cliente agenda. O estabelecimento acompanha.</h2>
          <p className="mt-4 text-lg leading-relaxed text-muted-foreground">Duas experiências conectadas à mesma agenda, reproduzidas a partir das telas reais do produto.</p>
        </div>

        <div className="mt-14 grid gap-6 xl:grid-cols-2">
          <article className="grid min-h-[720px] overflow-hidden rounded-[2rem] bg-[#e8f8f5] px-6 pt-10 sm:grid-cols-[.8fr_1.2fr] sm:px-10 sm:pt-12">
            <div className="relative z-10 pb-8 sm:pt-8">
              <p className="text-[11px] font-bold uppercase tracking-[.16em] text-[#178b65]">Estabelecimento demonstrativo</p>
              <h3 className="mt-3 font-display text-3xl tracking-[-.04em]">Veja o aplicativo funcionando</h3>
              <p className="mt-4 text-sm leading-relaxed text-[#5c6461]">Uma experiência completa para o cliente descobrir, agendar e acompanhar seus horários.</p>
              <div className="mt-6 flex flex-wrap gap-2" aria-label="Telas demonstradas">
                {CLIENT_SCREENS.map((label, index) => <button key={label} type="button" aria-pressed={screen === index} onClick={() => chooseScreen(index)} className={cn("rounded-full border px-3 py-2 text-[11px] font-semibold transition", screen === index ? "border-[#178b65] bg-[#178b65] text-white" : "border-[#b9d7cc] bg-white/60 text-[#395b4f]")}>{label}</button>)}
              </div>
            </div>
            <ClientPhone screen={screen} onPause={setPaused} />
          </article>

          <article className="grid min-h-[720px] overflow-hidden rounded-[2rem] bg-[#f0f1ef] px-6 pt-10 sm:grid-cols-[.72fr_1.28fr] sm:px-9 sm:pt-12">
            <div className="relative z-10 pb-8 sm:pt-8">
              <p className="text-[11px] font-bold uppercase tracking-[.16em] text-[#178b65]">A estrutura real do painel</p>
              <h3 className="mt-3 font-display text-3xl tracking-[-.04em]">A agenda do dia no bolso</h3>
              <p className="mt-4 text-sm leading-relaxed text-[#5c6461]">Indicadores, filtros, profissionais, horários e status no mesmo formato usado pelo estabelecimento.</p>
            </div>
            <OwnerPhone />
          </article>
        </div>
...
function ClientHome() {
  return <div className="h-full bg-[#0e0f12] px-3 pt-10 text-[#f8f5ef]">
    ...
    <div className="relative mt-4 h-[145px] overflow-hidden rounded-[1.4rem]"><Image src="/images/salon-hero-stylist-v2.webp" alt="Espaço Aurora, estabelecimento demonstrativo" fill quality={95} sizes="280px" className="object-cover object-center" /><div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent" /><div className="absolute bottom-3 left-3"><span className="rounded-full bg-[#7df89b] px-2 py-1 text-[7px] font-bold text-black">MISTO</span><strong className="mt-1.5 block text-base">Espaço Aurora</strong><small className="text-[7px] text-white/75">Jardins · São Paulo</small></div></div>
    <div className="mt-3 rounded-[1.3rem] bg-[#7df89b] p-3.5 text-[#0c130f]"><strong className="text-sm">Agendar um horário</strong><p className="my-1 text-[8px] opacity-70">Escolha o serviço e veja os horários disponíveis agora.</p><button className="rounded-full bg-[#111418] px-3 py-2 text-[8px] font-bold text-[#7df89b]">Agendar agora ↗</button></div>
